 ...
 */
public class OpenCVScanningActivityBack extends BaseActivity {
...
//            Mat dst= ImageUtil.rotation(inputFrame);
//            dst= faceDetector.detector(dst);
        }
    };
    ...
    CameraBridgeViewBase.CvCameraViewListener cvCameraViewListener=new CameraBridgeViewBase.CvCameraViewListener() {
        ...
        public Mat onCameraFrame(Mat inputFrame) {
            ...
            Mat dst1=new Mat();
            //转置
            Core.transpose(inputFrame,mat);
            if(cameraIsFront){
                Core.flip(mat,dst,0);
                Core.flip(dst,dst1,1);
//                scanning(dst1);
                faceDetector.detector(dst1);
                return dst1;
            }else {
                Core.flip(mat,dst,1);
//                scanning(dst);
                faceDetector.detector(dst);
                return dst;
            }
//            Mat dst= ImageUtil.rotation(inputFrame);
//            dst= faceDetector.detector(dst);
    ...
    private void addSetSimlaryMsg(final String msg){
        getHandler().post(new Runnable() {
            @Override
            public void run() {
                TextView textView = new TextView(OpenCVScanningActivityBack.this);
